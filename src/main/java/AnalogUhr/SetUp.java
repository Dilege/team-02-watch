...
public class SetUp implements Runnable {
	...
	public void draw() {
		...
		/*
		 * SetAuswahl.graph=g; SetAuswahl.buffer=buffer; SetAuswahl.s=size;
		...
		} else if (Memory.form2 == 1) {
			g.setColor(Color.black);
			g.fillRect(10, 10, size - 20, size - 20);
			g.setColor(Color.white);
			g.fillRect(20, 20, size - 40, size - 40);
		} else if (Memory.form3 == 1) {

		} else if (Memory.form4 == 1) {
			g.setColor(Color.black);
			g.fillRoundRect(10, 10, size - 20, size - 20, 200, 200);
			g.setColor(Color.white);
			g.fillRoundRect(20, 20, size - 40, size - 40, 180, 180);
		}
		...
		for (int i = 1; i <= 12; i++) {
			...
			radius = center - 60;
			angleX = (int) ((center) + ((Math.sin(position) * radius) - 10));
			angleY = (int) ((center) - ((Math.cos(position) * radius) - 10));
			g.setColor(farbe);
		...
		time = (System.currentTimeMillis() + (3600000 * Memory.timeZone))
				/ (60.0 * 12 * 60 * 1000.0) * Math.PI * 2;
		// 3600000 ms = 1 hours
